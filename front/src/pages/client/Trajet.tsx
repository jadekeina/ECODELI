...
const Trajet = () => {
    ...
    const updatePolyline = () => {
        if (!mapInstance.current || !departMarker.current || !arriveeMarker.current) return;
    ...
    useEffect(() => {
        ...
        if (departInputRef.current) {
            const autocompleteDepart = new google.maps.places.Autocomplete(departInputRef.current, options);
            autocompleteDepart.addListener("place_changed", () => {
                ...
                if (place.geometry?.location) {
                    ...
                    if (!departMarker.current) {
                        departMarker.current = new google.maps.Marker({ map: mapInstance.current });
                    }
                    departMarker.current.setPosition(location);
                    updatePolyline();
                    if (departMarker.current.getPosition() && arriveeMarker.current?.getPosition()) {
        ...
        if (arriveeInputRef.current) {
            const autocompleteArrivee = new google.maps.places.Autocomplete(arriveeInputRef.current, options);
    ...
    return (
        <div className="flex flex-col md:flex-row h-[calc(100vh-80px)] w-full">
            <div className="w-full md:w-1/3 p-6 space-y-4">
                <h2 className="text-xl font-semibold mb-4">Planifiez votre trajet</h2>
                ...
                <div className="space-y-2">
                    ...
                    <input
                        ...
                        onChange={(e) => {
                            ...
                            const now = new Date();
                            if (newDate < now) {
                                setErreurHeure("Veuillez choisir une date/heure future.");
                                return;
                            }
                            setErreurHeure("");
                    ...
                    />
                    {erreurHeure && <p className="text-sm text-red-500 italic">{erreurHeure}</p>}
                    ...
                    <textarea
                        ...
                        placeholder="Ex : Nous avons des bagages"
                        value={commentaire}
