...
async function authMiddleware(req, res, next) {
  try {
    ...
    // Suppression de la vérification du token BDD.
    // Le token est déjà vérifié par jwt.verify.
    db.getUserById(userId, (err, result) => {
      ...
      if (!result || !result.length) {
        console.log("AUTH_MIDDLEWARE: db.getUserById N'A PAS TROUVÉ l'utilisateur avec ID:", userId); // DEBUG
        return res.status(401).json({ message: "Authentification échouée: Utilisateur introuvable." });
      }

      const user = result[0];
      // console.log("AUTH_MIDDLEWARE: Utilisateur trouvé en BDD:", user.mail, "Token BDD (début):", user.token ? user.token.substring(0,20) + '...' : 'N/A'); // DEBUG - cette ligne peut rester ou être supprimée

      // Ligne suivante supprimée/commentée définitivement car elle cause le problème de mismatch
      // if (user.token !== token) {
      //   console.log("AUTH_MIDDLEWARE: Mismatch Token BDD vs Token fourni.");
